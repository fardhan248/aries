...
from postgres.store.base import BasePostgresStore
        ...
        *,
        ...
        if pool_config is not None:
            ...
            async with cast(
                ...
                yield cls(conn=pool, index=index, ttl=ttl, serde=serde)
        else:
            async with await AsyncConnection.connect(
                ...
                if pipeline:
                    async with conn.pipeline() as pipe:
                        yield cls(conn=conn, pipe=pipe, index=index, ttl=ttl, serde=serde)
                else:
                    yield cls(conn=conn, index=index, ttl=ttl, serde=serde)
