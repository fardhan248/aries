...
class BasePostgresStore(Generic[C]):
    ...
    _serde: EncryptedSerializer | None = None

    def _encrypt_value(self, value: dict) -> bytes:
        """Enkripsi dict value menjadi byte enkripsi"""
        if self._serde is not None:
            _, encrypted = self._serde.dumps_typed(value)
            return encrypted
        
        return orjson.dumps(value)
    
    def _decrypt_value(self, raw: bytes | orjson.Fragment) -> dict[str, Any]:
        """Dekripsi bytes jadi dict"""
        if self._serde is not None:
            if isinstance(raw, orjson.Fragment):
                raw = bytes(raw.buf) if hasattr(raw, "buf") else raw.contents
                if isinstance(raw, str):
                    raw = raw.encode()

            return self._serde.loads_typed(("encrypted", raw))
    ...
    def _prepare_batch_PUT_queries(
        ...
        if inserts:
            ...
            for op in inserts:
                encrypted = self._encrypt_value(cast(dict, op.value))
                insertion_params.extend(
                    (
                        _namespace_to_text(op.namespace),
                        op.key,
                        encrypted, #Jsonb(cast(dict, op.value)),
                    )
                )
    ...
    def _prepare_batch_search_queries(
        ...
        for idx, (_, op) in enumerate(search_ops):
            filter_params = []
            filter_clauses = []
            if self._serde is None and op.filter:
                if op.filter:
...
class PostgresStore(BaseStore, BasePostgresStore[_pg_internal.Conn]):
    ...
    def __init__(
        ...
        *,
        ...
        serde: EncryptedSerializer | None = None,
    ) -> None:
        super().__init__()
        self._serde = serde
        if serde is not None:
            self._deserializer = self._decrypt_value
        else:
